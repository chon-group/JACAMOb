...
try:
    while True:
        # -------------------------------------------------
        # Receive messages from EJasonArch
        # -------------------------------------------------
        try:
            message = client.recv(1024).decode().strip()
            if message:
                #print(f"[Body] Received from EJasonArch: {message}")

                if message.startswith("!"):
                    action = message[1:]
                    javino.sendMsg(duster, action)
        ...
        # -------------------------------------------------
        # Request apparatus perceptions
        # -------------------------------------------------
        javino.sendMsg(locomotion, "getPercepts")
        time.sleep(0.5)
        if javino.availableMsg(locomotion):
            percepts = javino.getMsg()
            #print(f"[Body] Locomotion percepts: {percepts}")
            client.sendall((percepts + "\n").encode())

        javino.sendMsg(duster, "getPercepts")
        time.sleep(0.5)
        if javino.availableMsg(duster):
            percepts = javino.getMsg()
            #print(f"[Body] Duster percepts: {percepts}")
            client.sendall((percepts + "\n").encode())
